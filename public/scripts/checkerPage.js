...
<td>₱${price.toFixed(2)}</td>
  ...
  async function loadCategories() {
    try {
      ...
      categories.forEach(cat => {
        ...
        fragment.appendChild(new Option(cat.name, value));
      });

      updateSelect.appendChild(fragment);
    } catch (error) {
  ...
  function openUpdateModal(product) {
    ...
    loadCategories().then(() => {
      // populate form fields after categories are loaded
